...
function switchTab(tabId) {
  ...
  // URL oppdatering når man bytter fane
  updateUrlFromUi();
}
...
function applyUrlParamsIfAny() {
  ...
  if (id) {
    ...
    // sørg for at URL samsvarer med UI
    updateUrlFromUi();
  }
}

// -------------------- Build holder details table (horisontal + zebra) --------------------
function renderHolderDetailsTable(containerId, holderId, permitIds) {
  ...
  // CSV state for current holder details view is stored elsewhere (we export the "rows" we render here)
  return { columns, rows };
}

// -------------------- View: Innehaver – gjeldende --------------------
document.getElementById("holderNowBtn").addEventListener("click", () => {
  ...
  // CSV: vi eksporterer detaljer-listen (samme som "Vis detaljer"-tabellen)
...
document.getElementById("permitNowBtn").addEventListener("click", () => {
  ...
  // CSV for tillatelse–eier: en rad
...
document.getElementById("permitHistBtn").addEventListener("click", () => {
  ...
  // CSV for permit history: flere rader
...
document.addEventListener("click", (e) => {
  ...
  document.getElementById(targetId).innerHTML =
    renderKeyValueTableHtml(detailsObj, `Vis detaljer for ${permitDisplay}`);
});
...
function wireInputClearAndEnter(inputId, onEnterClickId, clearFn) {
  ...
  input.addEventListener("input", (e) => {
    ...
    else updateUrlFromUi(); // oppdater lenke når man skriver
  });
...
(async function main() {
  try {
    ...
    // Bruk URL-parametre hvis de finnes (auto-søk)
    applyUrlParamsIfAny();
  } catch (e) {
    console.error(e);
